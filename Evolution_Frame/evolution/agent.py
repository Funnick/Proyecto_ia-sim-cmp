...
directions_actions = [MoveNorth(), MoveSouth(), MoveWest(), MoveEast()]
...
class Agent(Object_base):
    ...
    def __init__(
        self, pos_x=-1, pos_y=-1, genes=[], behavior=None, rules=[], states=[]
    ):
        """
    ...
    def in_limits(self, pos, perception):
        ...
        if (
            pos[0] >= perception[0]
            and pos[0] <= perception[1]
            and pos[1] >= perception[2]
            and pos[1] <= perception[3]
        ):
            return True
        else:
            return False
    ...
    def move(self, perception):
        ...
        while steps < max_steps:
            ...
            for move in range(4):
                new_cell = (
                    cell[0] + directions[move][0],
                    cell[1] + directions[move][1],
                )
                ...
                if self.in_limits((new_cell[0], new_cell[1]), limits):
                    ...
                    if (
                        world.cell_is_edge(new_cell[0], new_cell[1])
                        and self.food_eat_today == 0
                    ):
                        continue
                    ...
                    for rule in self.behavior.rules.values():
                        ...
                        if rule.to_move and rule.to_relevance:
                            best_move_probability += rule.to_relevance(
                                self
            ...
            if (
                self.food_eat_today > 0
                and world.map[best_move[2][0]][best_move[2][1]].is_edge
            ):
            ...
            if (
                not self.pregnant
                and best_move[2] in self.behavior.rules["couples"].elements
            ):
                plan.append(HaveSex())
    ...
    def see_around(self, world):
        ...
        return (world, (left_corner_x, right_corner_x, left_corner_y, right_corner_y))
